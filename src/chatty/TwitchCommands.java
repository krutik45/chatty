...
public class TwitchCommands {
    ...
    public void addNewCommands(Commands commands, TwitchClient client) {
        ...
        commands.add("mods", p -> {
            if (localUserIsBroadcaster(p.getRoom(), client)) {
                ...
                sendMessage(p.getChannel(), "/mods", Language.getString("chat.twitchcommands.mods")
                        + " (will only work for broadcaster after February 18, 2023)");
            }
        });
        commands.add("vips", p -> {
            if (localUserIsBroadcaster(p.getRoom(), client)) {
                ...
                sendMessage(p.getChannel(), "/vips", Language.getString("chat.twitchcommands.vips")
                        + " (will only work for broadcaster after February 18, 2023)");
            }
        });
        commands.add("raid", p -> {
     ...
     *                  is provided for your convenient, takes from the first
     *                  command parameter,
     *                  the provided listener needs to be called to handle the
     *                  result
     * @param append
     */
     ...
     * @param args       If this is null only an error message will be output,
     *                   without
     ...
     *                   request, the listener it provides must be called so the
     *                   result can be
     *                   handled
     * @param outputArgs
     */
